...
export const batchUpdateItemsTool = {
  ...
  description:
    'Update multiple items with PER-ITEM different data (serial PATCH /items/{collection}/{id}). Each item: { key, data, verify? }. By default (allow_partial_apply=false), apply runs all-or-nothing preflight: if any item fails verify/validation, the entire batch is aborted with zero writes. When BULK_REQUIRES_PLAN=true (default), dry_run=false is rejected — use dry_run=true then directus_apply_plan.',
  inputSchema: Input,
